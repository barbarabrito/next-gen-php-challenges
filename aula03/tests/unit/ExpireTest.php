...
final class ExpireTest extends PHPUnit\Framework\TestCase
{
    ...
    public function nonExistentMethodsDataProvider(): array
    {
        return [
            ['nonExistentMethod1'],
            ['nonExistentMethod2'],
    ...
    /**
     * @dataProvider nonExistentMethodsDataProvider
     */
    public function testExpiresComponentThrowMethodDoesNotExistException($nonExistentMethod)
    {
        $this->expectException(IntervalMethodNotFoundException::class);
        $this->expectExceptionMessage("Method '$nonExistentMethod' does not exists");
        
        $expires = new Expires();
        $expires->$nonExistentMethod(1);
    }
